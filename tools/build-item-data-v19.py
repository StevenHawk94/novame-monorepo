#!/usr/bin/env python3
"""Generate runtime item data from the v30 rules over stable memory icon IDs."""
...
MIGRATION = ROOT / "supabase" / "migrations" / "20260820000054_items_v30_catalog.sql"
QA_PATH = SOURCE_DIR / "items-v30-data-qa.json"
ICON_COUNT = 5439
NEW_FIRST_ROW = 5392
# v30 preserves all existing 5,390 identities and appends 49 bundled icons.
# The original v19 catalog migration is already deployed, so this generator
# refreshes bundled requires and writes only the appended rows to a new delta.
WRITE_IMAGE_MAP = True
WRITE_SQL_CATALOG = True
...
def main() -> None:
    ...
    for name, item in items_by_name.items():
        ...
        if baseline_dictionary:
            if item["row_number"] < NEW_FIRST_ROW and (
                not baseline or baseline.get("displayName") != name
            ):
                raise ValueError(f"Stable item identity changed at {item['id']}: {name!r}")
            if baseline and baseline.get("displayName") != name:
                raise ValueError(f"Item identity changed at {item['id']}: {name!r}")
    ...
    image_lines = [
        ...
        f" * One bundled 256px WebP per v30 memory item ({ICON_COUNT:,} total).",
        " */",
        "",
    ...
    # v30 Guided Prompt secondary tabs. First appearance determines tab order;
    ...
    values = []
    for row_number in sorted(row for row in items_by_row if row >= NEW_FIRST_ROW):
        item = items_by_row[row_number]
        ...
        values.append(
            "  (%s, %s, %d, %d, %s, %s, %s)"
            % (
                sql(item["id"]), sql(f"v30-{page_start}-{page_end}"), offset // 7, offset % 7,
    ...
    qa = {
        "schema": "memory-items-v30-data-qa@1",
