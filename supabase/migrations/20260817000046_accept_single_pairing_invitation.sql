-- Accept one invitation and establish the exclusive pairing atomically.
-- Any other pending invitation involving either newly paired user is ignored:
-- brand-new rows are removed; historical accepted rows that were reused for a
-- re-invite are restored to their accepted history state.
...
declare
  ...
  v_changed integer := 0;
begin
  -- Read once to discover the other party. Authorization is checked again
  -- after the per-user locks, so concurrent accepts cannot use stale state.
  ...
  -- Lock by USER (not merely by pair). A-B and A-C therefore serialize.
  ...
  -- Re-invites reuse an old accepted row. Preserve that historical marker.
  update public.friendships
     set status = 'accepted'
   where id <> p_friendship_id
     and status = 'pending'
  ...
  -- New pending requests have no historical relationship to preserve.
