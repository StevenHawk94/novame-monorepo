-- Memories / Theirs must be the current partner's Mine collection after both
-- memory-content and sharing filters are applied. Keep this aggregation in
-- Postgres so pagination is performed after privacy filtering and remains
-- correct for accounts with long histories.
...
as $$
  with visible_memories as (
    ...
    join public.reflect_items ri
      on ri.reflect_id = m.reflect_id
     and ri.user_id = m.user_id
     and ri.item_id = m.item_id
    ...
    where m.user_id = p_owner_user_id
      ...
      and (
        coalesce(
          nullif(btrim(m.description), ''),
          nullif(btrim(m.refined_desc), ''),
          nullif(btrim(m.raw_excerpt), '')
        ) is not null
      )
      and (
        not coalesce(p_require_reflect_share, true)
        or coalesce(r.shared_to_friends, true) = true
      )
  ), grouped as (
    select
      vm.item_id,
      count(*)::integer as count,
      min(vm.created_at) as first_seen_at
  ...
  where p_before_first_seen_at is null
     ...
     or (
       p_before_item_id is not null
       and g.first_seen_at = p_before_first_seen_at
       and g.item_id < p_before_item_id
     )
...
as $$
  ...
  where m.user_id = p_owner_user_id
    ...
    and (
      ...
      or (
        p_before_memory_id is not null
        and m.created_at = p_before_created_at
        and m.id < p_before_memory_id
      )
    )
