 ...
 *
 * Body: { name } → presigned PUT URLs for the outfit's 3 fixed asset keys.
 ...
 *
 * Content types are pinned in the signature: webp for the two images,
 * video/quicktime for the .mov — the browser must send the same
...
export async function POST(request) {
  ...
  try {
    ...
    const [thumb, bunny, video] = await Promise.all([
