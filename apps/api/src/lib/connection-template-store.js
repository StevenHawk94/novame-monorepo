...
export async function readConnectionTemplates(supabase, familyKeys) {
  ...
  const { data, error } = await supabase.from('connection_scenario_templates')
    .select('template_id,family_key,section,module_key,scenario_key,scenario,required_evidence,disqualifiers,emotional_weight,tone_mode,depth_level,field_pattern,label_options,template_card,blocked_overlap,library_version')
    .eq('active', true)
    .in('family_key', keys)
