...
export async function GET(request) {
  try {
    ...
    if (!token) {
      console.warn('[daily-tasks GET] rejected: no bearer token')
    ...
    if (_authErr || !_authUser) {
      console.warn('[daily-tasks GET] rejected: token verify failed', _authErr && _authErr.message)
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }
    if (_authUser.id !== userId) {
      console.warn('[daily-tasks GET] rejected: token user', _authUser.id, '!= userId', userId)
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }
...
export async function POST(request) {
  try {
    ...
    const token = authHeader.replace(/^Bearer\s+/i, '').trim()
    if (!token) {
      console.warn('[daily-tasks POST] rejected: no bearer token')
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }
    const _authSupabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL,
    ...
    const { data: { user: _authUser }, error: _authErr } = await _authSupabase.auth.getUser(token)
    if (_authErr || !_authUser) {
      console.warn('[daily-tasks POST] rejected: token verify failed', _authErr && _authErr.message)
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }
    if (_authUser.id !== userId) {
      console.warn('[daily-tasks POST] rejected: token user', _authUser.id, '!= userId', userId)
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }
    ...
    // === CREATE wisdom tasks ===
    // Note: the authoritative creation path is now publish-wisdom, which
    // calls createWisdomQuests bound atomically to a successfully created
    // card (so quests can't be lost to a client network error after a
    // successful publish). This route action is kept for parity and uses
    // the same shared helper so the row shape never drifts.
    if (action === 'create' && tasks && tasks.length > 0) {
      await createWisdomQuests(supabase, userId, tasks)
