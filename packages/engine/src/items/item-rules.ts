...
export interface ItemRule { keyword: string; item_id: string; action: 'enable' | 'disable' | 'never' | 'reset'; revision: number }
...
export function applyItemRules(base: ItemDictionary, rules: ItemRule[]): ItemDictionary {
  ...
  for (const rule of [...rules].sort((a, b) => a.revision - b.revision)) {
    ...
    } else if (rule.action === 'disable' || rule.action === 'never') {
